...
\noindent I metodi dichiarati possono, ovviamente, anche ricevere parametri, per la loro eventuale gestione ed elaborazione. Possiamo quindi introdurre il concetto di \textbf{parametri formali} e \textbf{parametri attuali}. I primi sono quelli dati all'inizio del metodo, mentre i secondi sono il risultato dell'elaborazione della funzione e sarà ciò che viene ritornato.
\begin{lstlisting}[language=Java]
	// Il metodo prende il parametro formale n
	public int incr (int n) {
		// res e' il parametro attuale elaborato
		int res = n+1;
		return res;
	}
	
	public void printIncr (int x) {
		int y = incr(x);
		System.out.println(y);
	}
\end{lstlisting}
\noindent Bisogna quindi prestare attenzione a quale dato viene ritornato dal metodo, perché se nella funzione incr ritornassimo n, tutta l'elaborazione sarebbe stata inutile.\par
Un'altro concetto molto importante è la parola chiave per far riferimento all'oggetto corrente: la keyword \textbf{this}, la quale può essere usata per attributi e variabili ed è particolarmente utile per evitare ambiguità fra gli oggetti.
\begin{lstlisting}[language=Java]
...
\end{lstlisting}
\noindent L'utilizzo di this è molto importante per rendere il codice chiaro ed è uno standard da rispettare. In assenza di this, il programma non avrebbe capito quale stringa nome prendere, causando errori di compilazione.
