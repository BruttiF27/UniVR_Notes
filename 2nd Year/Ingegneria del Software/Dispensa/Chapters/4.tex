...
l'assegnazione delle responsabilità nel software e costituiscono il fondamento per la progettazione di sistemi orientati agli oggetti. In particolare,
...
\noindent \textbf{Observer} è un altro pattern comportamentale che consente di definire un "meccanismo di iscrizione" al quale possono prendere parte
più oggetti, i quali saranno avvisati su ogni evento che concerne l'oggetto di loro interesse.\par
Immaginiamo di avere due oggetti: un negozio e un cliente; quest'ultimo è interessato in un particolare brand di prodotti che sarà presto reso
disponibile. Il cliente potrebbe visitare il negozio ogni giorno, ma è verosimile che compia giri a vuoto perché non ha la certezza della disponibilità.
Per risolvere questo fastidio, il negozio potrebbe inviare molte mail ai clienti per aggiornarli sullo stato della disponibilità, ma a coloro che non
sono interessati potrebbe risultare altrettanto fastidioso.\par
In questo contesto chiamiamo \textbf{publisher} l'oggetto di interesse con funzione di comunicare il suo stato e \textbf{iscritti} gli altri oggetti
interessati ai cambiamenti del primo. La soluzione di Observer suggerisce di aggiungere un meccanismo di iscrizione al publisher sicché altri oggetti
individuali possano iscriversi o disiscriversi dalla sua "newsletter". Il meccanismo è generalmente costruito con un array che salva i riferimenti agli
iscritti e vari metodi pubblici che consentono di iscrivere o disiscrivere gli oggetti.\newline

\noindent Terminiamo lo studio dei design pattern con \textbf{State}, il quale consente a un oggetto di alterare il suo comportamento quando cambia il
suo stato interno. Si tratta di una strategia relativa all'implementazione delle macchine a stati finiti (FSM).\par
Supponiamo di avere un'applicazione che funge da text editor, la quale può lavorare su documenti che possono presentare tre stati diversi: bozza,
moderazione e pubblicazione. A primo impatto si penserebbe di usare un costrutto condizionale per cambiare modalità, ma con l'aumentare dei diversi
stati, aumenterebbe di molto la complessità del codice.\par
La soluzione proposta da State suggerisce che si creino nuove classi per ogni stato di un oggetto e si estraggano tutti i comportamenti specifici dello
stato in esse. Quindi, al posto di implementare tutti i comportamenti in un singolo oggetto, detto \textbf{contesto}, lo si usa solo per salvare un
riferimento a uno degli oggetti di stato che rappresenta quello corrente, al quale si delega ogni funzione relativa. È bene che ogni stato implementi
