...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[i]
		...
		\State Inizializza il flusso $f$ a $0$
		\While{$\exists p\in G_f$ tale che sia aumentante}
		\State Aumenta il flusso $f$ lungo $p$
		\EndWhile
	\end{algorithmic}
\end{algorithm}
...
% TODO Presenta ford-fulkerson con pseudocodice, complessità e correttezza.
% TODO Presenta caso limite delle innumerevoli iterazioni e fornisci Karp come soluzione al problema
% TODO Presenta Karp e spiegane la complessità.
...
Sia f una funzione di flusso; sono equivalenti le seguenti proprietà:
    - f è un flusso massimo (non esistono flussi di modulo più alto)
    - G_f (rete residua) non ha cammini aumentanti.
    - f(u,v) = c(u,v) per un taglio (u,v) // esiste un taglio saturo

1\implies 2:
Se abbiamo trovato un flusso massimo, banalmente nella rete residua non esisteranno cammini aumentanti.

2\implies 3:
Se non ci sono cammini aumentanti nella rete residua, significa che dato un taglio, è stato trovato un arco la cui funzione di flusso ha valore
equivalente alla capacità massima.

3\implies 1:
Se esiste un taglio saturo, questo è il flusso massimo. Non è possibile infatti aumentare il flusso.

Se FF termina, abbiamo quindi trovato un flusso massimo. Ok, ma sto algoritmo termina? Generalmente no, lmao, perché di base si lavora con numeri
razionali e si continuerebbe ad aumentare il flusso di una quantità razionale sempre più piccola, avvicinandosi sempre di più al modulo massimo, ma
senza mai raggiungerlo.
Tuttavia, coi naturali la storia è diversa, perché nel peggiore degli scenari, incrementando sempre di 1 il valore, si otterrà il flusso massimo in
tempo equivalente al suo valore.

Complessità: O(|f*|(V+E))
f* è un flusso massimo
Il corpo ha complessità V+E



V+E è il numero di interazioni da attuare nell'algoritmo; se queste sono di un numero molto alto, FF risulterebbe estremamente inefficiente.
Introduciamo quindi Karp.

Supponiamo di usare BFS per la ricerca di un cammino aumentante.
Sia d_i la funzione distanza calcolata da BFS all'iterazione i.
\forall i < j, \forall u, d_i(u)\leq d_j(u). La funzione di distanza è monotona non decrescente.

Dimostrato questo lemma, quante volte è possibile che un arco (u,v) sia critico in un cammino aumentante?
Prendiamo l'arco e supponiamo che sia critico in un cammino aumentante.
u è a distanza d, mentre v a d+1 in quanto successore. In quanto critico, sparisce dalla rete residua. Per farlo ritornare al suo interno sarà
necessario trovare un altro arco critico, ma allora si avrebbe distanza d+2, cosa non possibile perché d è al massimo uguale al numero di archi del
grafo, V.
(v,u) quindi non può essere critico più di V/2 volte, e visto che per ogni cammino aumentante c'è un arco critico, non possiamo trovare cammini
aumentanti più di E*V/2 volte.

In questo contesto, la complessità di FF è O(EV(V+E)) ed è chiamato algoritmo di Karp; banalmente è un ford-fulkerson che usa la BFS per la ricerca del
cammino aumentante. In quanto tale, la complessità totale è data dal minimo di queste due.
O(min(EV(V+E), |f*|(V+E)))

Trovare un flusso di modulo massimo e trovare un taglio di capacità minima è la stessa cosa. È il teorema del taglio minimo e flusso massimo. Questo
permette di affrontare il problema da un punto di vista diverso.
\end{comment}