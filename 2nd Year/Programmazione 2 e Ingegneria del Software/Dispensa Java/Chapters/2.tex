...
\section{Classi innestate e interne}
...
\noindent$\bullet$ \textbf{Classe innestata}: Classe statica visibile ed utilizzabile a prescindere da quella in cui è dichiarata. Possono avere accesso ad ogni campo statico della classe primaria, indipendentemente dal modificatore d'accesso.
\begin{lstlisting}[language=Java]
	// Dichiarazione di classe innestata
	public class Outer {
		static class Nested { /* Blocco di codice */ }
	}
	
	// Istanziazione di un suo oggetto
	Outer.Nested nestedObject = new Outer.Nested();
\end{lstlisting}
\noindent$\bullet$ \textbf{Classe interna}: Classe non statica con le stesse dinamiche dell'innestata, ma il suo utilizzo dipende dalla primaria.
\begin{lstlisting}[language=Java]
	// Dichiarazione di classe interna
	public class Outer {
		class Inner { /* Blocco di codice */ }
	}
	
	// Istanziazione richiede un oggetto della classe primaria.
	Outer outerObject = new Outer();
	Outer.Inner innerObject = Outer.new Inner();
\end{lstlisting}
\noindent Una dinamica di ottimizzazione molto utilizzata è invece la dichiarazione di classi non statiche all'interno dei metodi. Queste sono dette \textbf{locali}, possono accedere esclusivamente a variabili finalizzate e vengono deallocate al termine del metodo. Generalmente ha senso utilizzarle anche per ridurre la complessità generale del codice in runtime, garantire una maggiore leggibilità e organizzazare al meglio le funzionalità del codice, dando anche limitazioni sullo scope.
\begin{lstlisting}[language=Java]
	public void outerMethod () {
		...
		System.out.println(localObject.addOne()); // Output: 2
	}
\end{lstlisting}