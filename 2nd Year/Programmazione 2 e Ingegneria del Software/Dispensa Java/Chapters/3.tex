...
\begin{lstlisting}[language=Java]
	...
	public class Pair <T> {
		...
		// Essendo generico, puo' essere qualunque oggetto.
	...
	// Classe utilizzabile con piu' tipi. Cast non necessario.
	Pair <String> p = new Pair <> ("one", "two");
	Pair <Integer> q = new Pair <> (1, 2);
\end{lstlisting}
\noindent I generics funzionano perché al posto del carattere il compilatore riconosce la classe Object, ed in quanto ogni altra classe è suo sottoinsieme, si effettua un downcast tautologico. L'uso del generic in questo modo lo rende un \textbf{parametro libero}, mentre se lo si usa per estendere altri parametri, sarà necessario dare un limite inferiore per evitare che il compiler riscontri problemi nella sua ricerca ricorsiva. T sarà chiamato \textbf{parametro legato}.
\begin{lstlisting}[language=Java]
	class Clazz <T extends S1 & S2 & ... & Sk>	// Limite superiore
	class Clazz <T super S>	// Limite inferiore
\end{lstlisting}
\begin{lstlisting}[language=Java]
	public class Pair <T extends Person> {
		...
		public T getFirst () { return first.getName(); } // Nessun errore
		public T getSecond () { return second; } // T estende Person, ok.
	}
\end{lstlisting}
