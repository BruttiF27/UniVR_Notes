...
\begin{itemize}
    \item \textbf{Addizione}: $z_1 + z_2 = (a + bi) + (c + di) = (a + c) + (b + d)i$\par
    Raccogli $i$ e somma tutto il resto.
    \begin{eg}
        $(6 + 7i) + (-12 + 17i) = (6 - 12) + (7 + 17)i = -6 + 24i$
    \end{eg}
    \item \textbf{Sottrazione}: $z_1 - z_2 = (a + bi) + (-c -di) = (a - c) + (b - d)i$\par
    Non dissimile dall'addizione, al sottraendo si negheranno gli elementi.
    \begin{eg}
        $(3+2i)-(1+i) = 2+i$
    \end{eg}
    \item \textbf{Moltiplicazione}: $z_1 × z_2 = (a + bi)(c + di) = ac + adi + bci + bdi^2 = (ac - bd) + (ad + bc)i$\par
    La classica moltiplicazione fra polinomi.
    \begin{eg}
        $(1-i)(3+2i) = 3+2i-3i-2i^2 = 5-i$
    \end{eg}
    \item \textbf{Divisione}: Una razionalizzazione, in sintesi.
    \begin{eg}
        $\dfrac{1 + 2i}{2 - i} = \dfrac{1 + 2i}{2 - i} \times \dfrac{2 - i}{2 - i} = \dfrac{2 + i + 4i + 2i^2}{4 + 2i - 2i - i^2} = \dfrac{8i}{8} = i$
    \end{eg}
\end{itemize}
Altri casi particolari di numeri sono:
\begin{itemize}
    \item \textbf{Opposto}: Si dice tale quando sommato algebricamente ad un altro numero riporta 0 come risultato. $z_1 + z_2 = 0$
    \item \textbf{Coniugato}: Il numero complesso avente la stessa parte reale e parte immaginaria di segno opposto rispetto a \textit{z}. $z = a + bi, \overline{z} = a - bi$
    \item \textbf{Modulo}: Il numero \textit{z} maggiore di 0 che vale $|z| = \sqrt{a^2+b^2}$
\end{itemize}
\begin{note}
...
\begin{itemize}
    ...
    \item $\dfrac{\overline{1}}{z_1} = \dfrac{1}{\overline{z_1}}, z \neq 0$
    \item $\dfrac{\overline{z_1}}{\overline{z_2}} = \overline{z \times \dfrac{1}{z_2}} = \overline{z_1} \times \dfrac{1}{z_2} = \overline{z_1} \times \dfrac{1}{\overline{z_2}}, z_2 \neq 0$
    \item $\dfrac{1}{z_1} = \dfrac{a - bi}{a^2 + b^2} = \dfrac{\overline{z_1}}{|z_2|^2}$ 
\end{itemize}
\end{note} 
...
$sin(\alpha) = \dfrac{b}{r}$, $cos(\alpha) = \dfrac{a}{r}$\par
$z = (rcos(\alpha)) + (rsin(\alpha))i = r(cos(\alpha) + isin(\alpha))$
\begin{itemize}
    \item $z_1 = (1, 0) = 1$ -> $cos(0) + isin(0)$
    \item $z_2 = (1, \dfrac{\pi}{2}) = i$ -> $cos(\dfrac{\pi}{2}) + isin(\dfrac{\pi}{2})$
    \item $z_3 = (1, \pi) = -1$ -> $cos(\pi) + isin(\pi)$
    \item $z_4 = (1, \dfrac{3}{2}\pi) = -i$ -> $cos(\dfrac{3}{2}\pi) + isin(\dfrac{3}{2}\pi)$
\end{itemize}
\end{minipage}
...
\begin{eg}
    $z_1 = r(cos(\alpha) + isin(\alpha))$, $z_2 = s(cos(\beta) + isin(\beta))$\newline
...
\begin{eg}
    ...
    $z = \sqrt{3}+i$\par
    $z = 2[cos(\dfrac{\pi}{6}) + isin(\dfrac{\pi}{6})] = z^6 = 2^6[cos(\pi) + isin(\pi)] = 2^6[-1 +i \times 0] = 2^6 \times -1 = -64$
\end{eg}
...
$z_n = \sqrt[n]{r} [cos(\dfrac{\alpha+k2\pi}{n}) + isin(\dfrac{\alpha+k2\pi}{n})$
...
\begin{itemize}
    \item $z_0 = \sqrt{-a}(cos(\dfrac{\pi}{2}) + isin(\dfrac{\pi}{2}) = i\sqrt{-a}$
    \item $z_1 = \sqrt{-a}(cos(\dfrac{3}{2}\pi) + isin(\dfrac{3}{2}\pi) = -i\sqrt{a}$
\end{itemize}
\end{minipage}
...
\begin{enumerate}
    ...
    Dato un numero complesso \textit{y}, esistono n radici n-esime complesse $z_0, z_1, ..., z_n$ di y. Inoltre, se in forma trigonometrica, le radici si presenteranno nella seguente forma per $k = 0, ..., n-1$: $z_k = \sqrt[n]{r} \times (cos(\dfrac{\alpha + k2\pi}{n}) + isin(\dfrac{\alpha + k2\pi}{n}))$
...
\section{Argomenti}
1.1 Insiemi di numeri
1.1 Numeri immaginari
1.1 Operazioni tra numeri complessi (addizione, moltiplicazione, sottrazione, divisione + teorema
fondamentale dell’algebra)
1.1 Coniugato e modulo
1.2 Coordinate polari
1.2 Forma trigonometrica di un numero complesso
1.2 Prodotto di numeri complessi in forma trigonometrica
1.3 La Formula di De Moivre
1.3 Definizione: radici n-esime
1.3 Teorema sulle radici n-esime
1.3 Radici quadrate di numeri reali negativi