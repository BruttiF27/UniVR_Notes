...
Proviamo adesso ad estendere il concetto e gestirlo su più bit. Supponiamo di avere una matrice $1024b\times32b$, quindi $1024$ parole e righe totali con $32$ colonne di bits.\par
...
Una memoria ram è qunidi un vettore di parole.




// What if, 1Mb x 64b?
1Kb = $2^10$ -> 1Mb = $2^20$ (20b bus indirizzi)

Essendo che le parole son fatte da 32b massimo, per gestire 64b avremo bisogno di coppie di circuiti S-ram.
Prendi il circuito di prima 1024x32 e sdoppialo. Il primo gestirà i bit più significativi e l'altro quelli meno sign.

Siccome hai richiesto 1M, 1M = 1024K.
Quindi serviranno 1024 righe con 2 colonne contenenti sti due chip.

Per selezionare il bit corretto si userà chip select, dato da un decoder supplementare che prende in input i bits che vanno da 10 a 19.
Dove vanno gli indirizzi meno significativi? Ma agli altri decoder per ogni riga, ovviamente.

- D-RAM
Abbiamo la wordline ed una sola bitline.
La cella da 1b è fatta in maniera diversa:
- Abbiamo un transistor collegato ad un condensatore. Quando è in conduzione il valore della cella è porteto

Con una certa frequenza inferiore al tempo di scarica, vado a leggere tutti i bit. Se vedo che un bit è a 1, lo riscrivo, mentre se è a 0 lo lascio stare.
Il refresh lo fa il circuito della memoria e scarica il valore nei registri.
