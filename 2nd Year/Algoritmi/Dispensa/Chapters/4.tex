...
\noindent Introduciamo adesso il concetto di \textbf{capacità residua} di un arco: il valore di capacità rimanente quando un arco fa scorrere un flusso.
Per esempio, se ha una base di capacità di $20$ unità e gli è inviato un flusso di $12$, la rimanente sarà uguale a $8$. Si definisce formalmente come: \[
	c_f(u,v) = c(u,v) - f(u,v)
\]
...
% TODO Rivedere il concetto di rete residua perché boh.
...
\begin{comment}
In presenza di due nodi collegati bidirezionalmente, è possibile che uno respinga il flusso dati dall'altro. Nell'IM1, a e c presentano questa
situazione; dove f(c,a)=1, è possibile che a respinga quanto ricevuto e lo rimandi a c, facendo valere come capacità residua 10+1=11.
In generale, se un nodo respinge un flusso, fa valere la sua capacità residua equivalentemente al flusso ricevuto. Se t deve respingere il flusso
di b, ritorna capacità residua 15.
La capacità residua è il flusso aggiuntivo che posso rispedire lungo ogni arco.

Esiste un percorso da s a t lungo il quale sia possibile aumentare il flusso? La risposta si trova con il cammino aumentante; un cammino da s a t nella
rete residua (rete che contiene le capacità residue). Nell'immagine è s, c, b, t. È evidente però come presenti un bottleneck tra c e b. Per questa
ragione possiamo ridirezionare il flusso uscente f(b,c) e sommarlo a quello f(b,t). Questa operazione mantiene i vincoli di conservazione perché quella
che stiamo facendo è un'operazione di ridirezionamento, non creazione o rimozione del flusso.
In tal caso, f(b,c)=0 e f(b,t) = 15+5 = 20. Flusso aumentato. Le capacità residue cambiano di conseguenza, costruendo una nuova rete residua.
Questo procedimento va ripetuto fino a quando il flusso inviato all'altra parte del taglio è quanto più vicino al valore della capacità massima. Quando
i due valori sono equivalenti, si dice che l'arco è saturo e si ha un flusso massimo.

--- Ford-Fulkerson
Parti dal flusso nullo, questa soddisferà tutte le proprietà e non ci saranno punti di accumulo. Finché nella rete residua esistono cammini aumentanti,
sia p un cammino aumentante, aumenta il flusso lungo p di una quantità pari alla capacità residua dell'arco critico (bottleneck).
...
Riconducibile ad un flusso massimo con una complessità relativamente bassa.
\end{comment}