...
Intuitivamente, il problema della ricerca del cammino minimo da più nodi sorgente richiede l'applicazione ripetuta della ricerca del cammino minimo da
una sorgente per ogni nodo del grafo. L'input rimane infatti un grafo orientato $G = (V, E)$ pesato con la relativa funzione peso $w:E\to\mathbb{R}$.
L'output è invece presentato in forma tabulare in cui l'elemento nella riga $u$ e nella colonna $v$ indica il peso di un cammino minimo tra questi due
nodi.\par
Data questa forma del risultato, si preferisce una rappresentazione con matrici di adiacenza $W = (w_{ij})$ di dimensioni $n\cdot n$, le quali salvano
i pesi degli archi del grafo orientato. In particolare, definiamo il peso $w_{ij}$ come: \[
	w_{ij} = \begin{cases}
		0 & \text{Se $i=j$}\\
		\text{Peso dell'arco orientato $(i,j)$} & \text{Se $i\neq j$ e $(i,j)\in E$}\\
		\infty & \text{Se $i\neq j$ e $(i,j)\notin E$}
	\end{cases}
\]
\noindent Nel secondo caso della definizione è definito il cammino minimo $\delta(i, j)$ dal vertice $i$ a $j$. Una soluzione completa ritorna questi
dati, ma anche una matrice dei predecessori $\Pi = (\pi_{i,j})$, dove $\pi_{ij}=NIL$ se $i=j$ o non esiste un cammino fra i due nodi, altrimenti prende
il valore del predecessore di $j$ in qualche cammino da $i$.\par
Il sottografo indotto da questa matrice $G_{\pi, i}$ è un albero di cammini minimi, ed in quanto tale è possibile utilizzare una procedura adattata di
printPath() per poter selezionare l'elemento della matrice di cui stampare il cammino minimo.\newline

\noindent Questo problema è particolarmente complesso perché non esistono ancora algoritmi per fornire una soluzione in tempi ottimi. Se applichiamo la
prima idea in presenza di un grafo con anche pesi negativi, dovremmo utilizzare Bellman-Ford, di complessità $O(VE)$, un totale di $|V|$ volte,
facendo risultare una complessità totale di $O(V^2E)$.

\begin{algorithm}[h]
	\caption{all\_pairs\_sp(G, w)}
	\begin{algorithmic}[i]
		\Require $G$: Grafo orientato pesato, $w$: Peso
		\State $W\gets init(G, w)$ \Comment{Pone a 0 gli elementi sulla diagonale e a $\infty$ altre posizioni}
		\For{$l\gets 1$ to $|V[G]|-1$} \Comment{Per ogni istanza del problema, quindi $|V|-1$ volte}
		\State $W^l\gets extend\_sp(W^{l-1}, w)$ \Comment{Rilassa tutti gli archi relativi al percorso}
		\EndFor
		\State $W^{|V[G]|}\gets extend\_sp(W^{|V[G]|-1}, w)$
		\If{$W^{|V[G]|}\neq W^{|V[G]|-1}$} \Comment{Controlla esistenza di cicli negativi}
		\State \textbf{return} false
		\EndIf
		\State \textbf{return} $W^{|V[G]|}$
	\end{algorithmic}
\end{algorithm}

\noindent Notare che si sta inizializzando la matrice alla sua forma di identità, ma con un'algebra diversa. Normalmente, una matrice identità presenta
valori uguali ad $1$ esclusivamente sulla sua diagonale principale, mentre tutte le altre posizioni sono poste a $0$; lo stesso procedimento è applicato
qui, ma con $0$ e $\infty$ rispettivamente. Questa forma matriciale consente l'applicazione di operazioni di somma e ricerca del minimo, per le quali
valgono le stesse proprietà già discusse.
\begin{algorithm}[h]
	\caption{extend\_sp(W, w)}
	\begin{algorithmic}[i]
		\Require $W$: Matrice di adiacenze, $w$: Peso
		\State $n\gets row[W]$ \Comment{Otteniamo la dimensione della matrice quadrata}
		\State Sia $W'$ matrice quadrata $n\times n$ con elementi $w'_{ij}$
		\State
		\For{$i\gets 1$ to $n$} \Comment{Per ogni sorgente $i$}
		\For{$j\gets 1$ to $n$} \Comment{Per ogni destinazione $j$}
		\State $w'_{ij}\gets \infty$ \Comment{Distanza iniziale da ridefinire esplorando il grafo}
		\State
		\For{$k\gets 1$ to $n$} \Comment{Per ogni possibile sorgente verso $j$}
		\State $w'_{ij}\gets min(w'_{ij}, w_{ik}+w_{kj})$ \Comment{Verifica peso migliore rispetto a $w'_{ij}$}
		\EndFor
		\EndFor
		\EndFor
		\State \textbf{return} $W'$ \Comment{La matrice con ogni cammino minimo}
	\end{algorithmic}
\end{algorithm}

\noindent Analizziamo la complessità di questo algoritmo: il cammino minimo è calcolato per un totale di $p = |V|-1$ volte e l'algoritmo di estensione
presenta ben tre cicli for eseguiti $|V|$ volte l'uno, portando la complessità totale a $O(V^4)$.\par
Trattandosi di un valore particolarmente alto, si ritiene necessario l'applicazione di tecniche specifiche per ridurlo. Una prima idea da un punto di
vista pratico sarebbe la parallelizzazione dell'algoritmo, dove i cammini sono calcolati concorrentemente, e al termine della procedura si unirebbero
i risultati, introducendo un fattore logaritmico e ottenendo una complessità totale di $O(V^3log(n))$.\par
Un'altra idea è fornita dall'algoritmo di \textbf{Floyd-Warshall}, una soluzione basata sulla programmazione dinamica che contempla archi di peso
negativo, ma non cicli negativi. L'algoritmo considera i nodi intermedi $k_i$ di un cammino minimo, definiti come un qualsiasi vertice del cammino non
ai suoi estremi: \[
p = \left<v_1, v_2, ..., v_l\right>;\quad k_i \neq v_1 \land k_i\neq v_l
\]
\noindent Numerando i nodi del grafo con $V = \{1, 2, ..., n\}$ prendiamo un sottoinsieme di vertici $\{1, 2, ..., k\}$ per qualche $1\leq k\leq n$. Per
una coppia di nodi qualsiasi $i,j$ prendiamo tutti i cammini minimi i cui vertici intermedi si trovano all'interno del sottoinsieme e definiamo $p$ il
peso del cammino minimo fra di loro.\par
Sfruttando la relazione tra $p$ e i cammini minimi da $i$ a $j$ i cui vertici intermedi si trovano dentro $\{1, 2, ..., k-1\}$, possiamo dire: 
\begin{itemize}
	\item \textbf{k non è vertice intermedio di p}: Tutti i vertici intermedi di $p$ appartengono all'insieme $\{1, 2, ..., k-1\}$ e un cammino
	minimo formato da questi nodi sarà un cammino minimo anche dell'insieme $\{1, 2, ..., k\}$, in quanto appartenenti al sottoinsieme.
	\item \textbf{k è vertice intermedio di p}: Spezziamo il cammino minimo intero in due parti; la prima $p_1$ da $i$ a $k$, la seconda $p_2$ da $k$ a
	$j$. Ciò forma due cammini minimi $\delta(i, k)$ e $\delta(k, j)$ i cui vertici intermedi appartengono a $\{1, 2, ..., k\}$. Inoltre, dato che $k$
	non è intermedio per nessuna delle due parti, possiamo dire che tutti i nodi intermedi di $p_1$ appartengono a $\{1, 2, ..., k-1\}$, rendendo $p_1$
	un cammino minimo coi vertici intermedi in questo insieme. Analogamente, $p_2$ è cammino minimo da $k$ a $j$ i cui vertici intermedi sono compresi
	nell'insieme $\{1, 2, ..., k-1\}$.
\end{itemize}

\begin{algorithm}[h]
	\caption{floyd-warshall(W, n)}
	\begin{algorithmic}[i]
		\Require $W$: Matrice di adiacenze, $n$: Dimensione matrice
		\State $D^0\gets W$ \Comment{Matrice iniziale contiene i pesi di $W$}
		\For{$k\gets 1$ to $n$}
		\State Sia $D^{k}=(d_{ij})$ matrice di dimensione $n\times n$
		\For{$i\gets 1$ to $n$}
		\For{$j\gets 1$ to $n$}
		\State $d_{ij}^k\gets min\left(d_{ij}^{k-1}, d_{ik}^{k-1} + d_{kj}^{k-1}\right)$ \Comment{Calcola cammino minimo tra le parti}
		\EndFor
		\EndFor
		\EndFor
...
\noindent L'algoritmo di Floyd-Warshall ha complessità totale di $\Theta(V^3)$, come si può vedere dai tre cicli for al suo interno. Lo scopo è rendere
sempre più deboli i vincoli tra i percorsi, fin quando non si arriverà alla soluzione dei quali è priva. Si ritorna dunque una matrice di adiacenza con
i costi dei cammini minimi per ogni sorgente.\par
Dipendentemente dal caso preso in esame, è possibile ridurre ulteriormente il tempo totale di esecuzione; l'algoritmo di \textbf{Johnson} funziona bene
in presenza di grafi sparsi, con una complessità di $O(V^2log(V) + VE)$, sicuramente migliore di Floyd-Warshall, ma risulta inefficiente se applicato
a grafi completi. La procedura prende in input un grafo orientato pesato $G$ con la relativa funzione peso $w$ e contempla anche l'esistenza di cicli
negativi, utilizzando una tecnica di \textbf{ricalcolo dei pesi} da un valore $w$ a $\hat{w}$. Servendosi infatti dell'algoritmo di Dijkstra come
sub-routine, è necessario garantirne il funzionamento portando tutti i pesi del grafo ad un valore positivo mantenendo i cammini minimi di $G$.\par
Si calcola quindi un nuovo insieme di pesi sui quali è possibile applicare Dijkstra, il quale deve rispettare le seguenti proprietà: \begin{itemize}
	\item Per ogni coppia di vertici $u,v\in V$, un cammino $p$ è minimo tra i due con la funzione peso $w$ se e solo se lo rimane anche con la funzione
	peso ricalcolata $\hat{w}$.
	\item Per ogni arco $(u,v)$, il nuovo peso $\hat{w}(u,v)$ non è negativo.
\end{itemize}

\noindent È quindi necessario dimostrare che il ricalcolo non cambia i cammini minimi del grafo originale e che rende tutti i pesi non negativi. Verrà
usato il seguente lemma: \begin{lemma}
	\textbf{Ricalcolare i pesi non cambia i cammini minimi}\par
	\noindent Supponiamo un grafo orientato pesato $G$ con relativa funzione peso $w$. Sia una funzione generica $h:V\to\mathbb{R}$ che associa i nodi a
	valori reali. Per ogni arco $(u,v)\in E$ definiamo: \[
	\hat{w}(u,v) = w(u,v) + h(u)-h(v)
	\]
	\noindent Sia adesso $p = \left<v_0, v_1, ..., v_k \right>$ un cammino qualsiasi dal nodo $v_0$ a $v_k$. Questo sarà un cammino minimo per loro due
	con un peso $w$ se e solo se $p$ rimane un cammino minimo con il peso $\hat{w}$, dunque: \[
	w(p) = \delta(v_0, v_k) \iff \hat{w}(p) = \hat{\delta}(v_0, v_k)
	\]
	\noindent Inoltre, $G$ ha un ciclo di peso negativo con funzione $w$ se e solo se ce l'ha anche con $\hat{w}$.
\end{lemma}

\noindent È stata quindi definita una funzione di potenziale; dimostriamone la veridicità supponendo il cammino $p = \left<v_0, v_1, ..., v_k\right>$:
\begin{equation}
	\begin{split}
		\hat{w}(p) &= \hat{w}(v_0, v_1) + \hat{w}(v_1, v_2) + ... + \hat{w}(v_{n-1}, v_k)\\
			&= w(v_0, v_1) + h(v_0) - h(v_1)\\
			&\quad+ w(v_1, v_2) + h(v_1) - h(v_2)\\
			&\quad+ ... + w(v_{n-1}, v_k) + h(v_{n-1}) - h(v_k)\\
			&= w(p) + h(v_0) - h(v_k)
	\end{split}
\end{equation}

\noindent Fra tutti gli elementi si annullano quelli intermedi; in forma semplificata con la sommatoria risulta quindi: \[
\sum_{i=1}^k w(v_i, v_{i+1}) + h(v_0) - h(v_k) = w(p) + h(v_0) - h(v_k)
\]
\noindent Dunque per un cammino qualsiasi $p$ da $v_0$ a $v_k$ vale l'equazione del lemma e siccome $h(v_0)$ e $h(v_k)$ non dipendono dal cammino, se
un percorso tra questi due nodi è più corto di un altro con la funzione $w$, allora lo sarà allo stesso modo con $\hat{w}$, facendo valere: \[
w(p) = \delta(v_0, v_k) \iff \hat{w}(p) = \hat{\delta}(v_0, v_k)
\]
\noindent Infine, in presenza di un ciclo, facendo valere $v_0 = v_k$, si rimuovono questi due valori dall'equazione, facendo risultare come risultato
$\hat{w} = w(p)$, dimostrando che il peso non cambia. Adesso è necessario garantire che la seconda proprietà sia valida, quindi $\hat{w}(u, v)$ deve
essere non negativo per ogni arco $w(u,v)\in E$.\par
Sarà necessaria la creazione di un nuovo grafo $G' = (V', E')$ per cui $V' = V \cup \{s\}$, ovvero a cui è aggiunto un nuovo supernodo $s$. Estendiamo
come prima cosa la funzione peso assegnando $w(s, v) = 0$ per ogni $v\in V$ e supponiamo che $G$ e $G'$ non abbiano cicli negativi, definendo
$h(v) = \delta(s,v)$ per ogni $v\in V'$. Grazie alla proprietà di disuguaglianza triangolare, vale per tutti gli archi di $E'$: \[
h(v) \leq h(u) + w(u,v)
\]
\noindent Definendo quindi i nuovi pesi secondo l'equazione del lemma sul ricalcolo, risulta: \[
\hat{w}(u,v) = w(u,v) + h(u) - h(v) \geq 0
\]
\noindent Rendendo di fatto tutti gli archi di peso non negativo senza modificare la semantica del grafo. L'algoritmo riceve in input un grafo e la
sua funzione del peso, per ritornare l'eventuale presenza di un ciclo negativo oppure la matrice contenente i cammini minimi per ogni sorgente.

\begin{algorithm}[h]
	\caption{johnson(G, w)}
	\begin{algorithmic}[i]
		\Require $G$: Grafo orientato pesato, $w$: Peso
		\State Sia $G'$ t.c. $V[G']\gets V[G]\cup \{s\}$, $E[G']\gets E[G]\cup \{(s,v):v\in V[G]\}$
		\State $w(s, v)\gets 0$ per ogni $v\in V[G]$
		\State
		\If{bellman-ford(G', w, s) = false}
		\State print(Grafo contiene un ciclo negativo)
		\Else
		\ForAll{$v\in V[G']$}
		\State $h(v)\gets \delta(s,v)$ \Comment{Cammino minimo calcolato con Bellman-Ford}
		\EndFor
		\ForAll{$(u,v)\in E[G']$}
		\State $\hat{w}(u,v)\gets w(u,v) + h(u) - h(v)$ \Comment{Calcolo del nuovo peso}
		\EndFor
		\State Sia $D = (d_{uv})$ una nuova matrice $n\times n$
		\State
		\ForAll{$u\in V[G]$}
		\State Esegui $dijkstra(G, \hat{w}, u)$ per calcolare $\hat{\delta}(u,v)$ per ogni nodo.
		\ForAll{$v\in V[G]$}
		\State $d_{uv}\gets \hat{\delta}(u,v) + h(v) - h(u)$
		\EndFor
		\EndFor
...
\noindent Analizziamo la complessità dell'algoritmo: viene eseguito in primo luogo Bellman-Ford, il cui costo complessivo è $O(VE)$; se è presente un
ciclo di peso negativo, l'algoritmo termina riportando l'istanza. Alternativamente, si assegna alla variabile $h(v)$ il peso del cammino minimo
$\delta(s, v)$, calcolato da Bellman-Ford per ogni nodo del neo-grafo, quindi $|V|$ volte.\par
Dopodiché si attua Dijkstra per un totale di $|V|$ volte, contribuendo con $O(VElog(V))$ al tempo totale, implicando che sia implementato con un heap.
Infine è assegnato il cammino minimo ad ogni elemento della matrice $D$. Per quanto riguarda grafi sparsi, abbiamo che $E = O(V)$, facendo risultare il
tempo totale di esecuzione: $O(V^2log(V))$, asintoticamente migliore di Floyd-Warshall.

%

\section{Flusso massimo}
Il problema della ricerca del \textbf{flusso massimo} è descritto tramite grafi e si pone come obiettivo il calcolo del percorso con il quale si ha
la velocità massima di scorrimento del materiale da un nodo sorgente ad uno di destinazione, detto \textbf{pozzo}. Per raggiungere questo obiettivo
sarà necessario deviare il flusso di materiale rispettando i vincoli di \textbf{capacità}.\par
I grafi utilizzati in questo contesto sono chiamati \textbf{reti di flusso} $G = (V, E)$ e si definiscono con una funzione di capacità associata a ogni
arco: \[
	c:V\times V\to \mathbb{R}^{\geq 0} \text{ tale che } \forall (u,v), (u,v)\notin E \implies c(u,v) = 0
\]
\noindent Che indica la massima portata che può scorrere in quell'arco e se non appartiene al grafo, equivarrà a zero. Supponendo ora un grafo, la
segnatura utilizzata per gli archi è [flussoEntrante/capacità], il cui primo valore ha segno variabile in base alla prospettiva del nodo. Osservando
l'immagine della rete di flusso è possibile vedere che dal vertice $a$ al $b$ sono inviate $12$ unità di materiale, mentre viceversa, ne sono inviate
$-12$. Questo è dovuto ai \textbf{vincoli} di capacità, i quali devono essere rispettati per ottenere una soluzione valida al problema: \begin{itemize}
	\item \textbf{Invariante della capacità}: Il valore del flusso deve essere sempre minore o uguale della capacità dell'arco. \[
		\forall u,v . f(u,v) \leq c(u,v)
	\]
	\item \textbf{Relazione di antisimmetria}: Il segno del flusso dipende dalla prospettiva. \[
		\forall u,v . f(u,v) = -f(v,u)
	\]
	\item \textbf{Regola di conservazione}: Non è possibile creare o rimuovere flusso; la somma dei flussi uscenti deve essere uguale a quella degli
	entranti. \[
		\forall v\in V - \{s, t\} \text{ . } \sum_v f(u,v) = 0
	\]
\end{itemize}

\noindent Adesso effettuiamo un taglio $U,V \text{ t.c } s\in U e t\in V$ sulla rete per calcolare il valore di capacità $c(U,V) = \sum_{u,v} c(u,v)$,
con $u\in U$ e $v\in V$, e il valore della funzione di flusso. Risulta che: \begin{itemize}
	\item La sezione di sinistra è capace di inviare alla destra un massimo di $12+14=26$ unità e invia effettivamente un flusso di $12+11-4=19$ unità.
	\item La sezione di destra è capace di inviare alla sinistra un massimo di $9$ unità e invia effettivamente un flusso di $4-12-11=-19$ unità.
\end{itemize}
\noindent Naturalmente, per il calcolo dei valori totali stiamo considerando esclusivamente gli archi sicuri e che sono state rispettate le tre regole
per le reti di flusso: non si è sforata la capacità, è stata segnalata l'antisimmetria ed il flusso è stato conservato.\par
Una proprietà molto utile delle reti di flusso riguarda che per un qualunque taglio eseguito nel grafo, passerà sempre lo stesso valore di flusso che è
uscito dalla sorgente. È dimostrabile per induzione sui tagli; infatti: \begin{itemize}
	\item \textbf{Base}: Se $s$ è l'unico nodo del grafo, la proprietà è banalmente dimostrata.
	\item \textbf{Induzione}: In presenza di più nodi, si effettua in primo luogo un taglio nel grafo per i vertici adiacenti alla sorgente. In quanto
	tali, il valore di flusso calcolato dagli archi sicuri è mantenuto e, continuando a spostare il taglio verso l'altra sezione, si terranno in
	considerazione solo questi ultimi, che a loro volta manterranno il quantitativo uscito dalla sorgente.
\end{itemize}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Images/streamNet.png}
	\caption{Rete di flusso}
\end{figure}

\noindent Il concetto duale della rete di flusso è la \textbf{rete dei residui}, un grafo indotto dalla prima il cui scopo è indicare quanto flusso
può essere ancora gestito nei vari archi. Per annotarlo si fa uso del concetto di \textbf{capacità residua}, intuitivamente il valore di capacità
rimanente quando nella connessione scorre un flusso, e formalmente descritta come: \[
	c_f(u,v) = \begin{cases}
	...
	\end{cases}
\]
\noindent La scrittura delle reti residue richiede necessariamente di prendere quella di flusso come punto di partenza; in particolare, possiamo avere
due possibilità per indicare le connessioni tra i nodi: \begin{itemize}
	\item Se tutta la capacità dell'arco è utilizzata la rete residua annota l'arco tra i due nodi con direzione opposta a quella originale e indica
	come valore il flusso ricevuto.
	\item Se non tutta la capacità dell'arco è utilizzata la rete residua annota due archi tra i nodi; il primo, nella medesima direzione della rete
	originale, indica il valore di flusso rimanente che potrebbe essere inviato, mentre il secondo, nella direzione opposta, indica il flusso ricevuto.
\end{itemize}
\noindent Prendiamo come esempio la rete di flusso discussa prima; il primo caso si rispecchia nel flusso $f(a,b)$; tutta la capacità è utilizzata e
quindi la rete residua annoterà una sola freccia in direzione opposta con il medesimo valore. Il secondo caso, invece, si può vedere con $f(c,d)$.
L'arco della capacità rimanente avrà valore $3$, mentre quello in direzione inversa presenterà $11$. Questa dinamica ci consente di manipolare il
flusso e ridirezionarlo per ricercare cammini che lo possano eventualmente massimizzare, detti \textbf{cammini aumentanti}. Il processo di ricerca si
effettua fintanto che è possibile trovarne di nuovi, e al suo termine si ottiene il flusso massimo.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Images/residualNet.png}
	\caption{Rete residua}
\end{figure}

\noindent Quest'idea è alla base dell'algoritmo di \textbf{Ford-Fulkerson}, il quale, a partire da un flusso nullo, continuerà a ricercare un cammino
aumentante $p$ nella rete residua e aumentare il flusso lungo di esso di una quantità pari alla sua capacità residua.
\begin{algorithm}[h]
	\caption{ford-fulkerson(G, s, t)}
	\begin{algorithmic}[i]
		\Require $G$: Rete di flusso, $s$: Sorgente, $t$: Pozzo
		\ForAll{arco $(u,v)\in E[G]$}
		\State $f[(u,v)]\gets 0$
		\EndFor
		\While{esiste un cammino $p$ da $s$ a $t$ nella rete residua $G_f$}
		\State $c_f(p) \gets min\{c_f(u,v) \text{ t.c. } (u,v)\in p\}$ \Comment{Prendi capacità residua minima}
		\ForAll{arco $(u,v)\in p$}
		\If{$(u,v)\in E[G]$} \Comment{Se l'arco in rete, somma la sua capienza al flusso}
		\State $f[(u,v)]\gets f[(u,v)] + c_f(p)$
		\Else $\text{ }f[(v,u)]\gets f[(v,u)] - c_f(p)$
		\EndIf
		\EndFor
		\EndWhile
...
\begin{teorema}
	\textbf{Correttezza di Ford-Fulkerson}\par
	\noindent Data una rete di flusso con una funzione di flusso e due nodi sorgente e pozzo, l'algoritmo di Ford-Fulkerson calcola il flusso massimo
	tra di loro in tal modo che: \begin{enumerate}
		\item $f$ è un flusso massimo e non esistono altri flussi di modulo più alto.
		\item La rete residua $G_f$ non ha cammini aumentanti.
		\item Per un taglio $(u,v)$ vale $f(u,v) = c(u,v)$, rendendolo saturo.
\end{enumerate}
\end{teorema}

\noindent Procediamo con una dimostrazione "a tutto tondo" del teorema: \begin{itemize}
	\item [$1\implies 2$]: Se è stato trovato il flusso massimo, banalmente nella rete residua non esisteranno cammini aumentanti.
	\item [$2\implies 3$]: Eseguiamo un taglio $(u,v)$ nella rete residua $G_f$ e definiamo $S$ l'insieme dei nodi appartenenti alla sezione della
	sorgente che sono raggiungibili da essa. Non esistendo cammini aumentanti, vale che il pozzo $t\notin S$ e per l'arco sicuro $(u,v)$ deve valere
	$f(u,v) = c(u,v)$, rendendolo saturo.
	\item [$3\implies 1$]: Se esiste un taglio saturo, questo è il flusso massimo. Non è dunque possibile aumentarlo.
\end{itemize}
\noindent L'algoritmo è utilizzabile anche con i numeri razionali, tuttavia, in tal caso non potrebbe terminare, perché la ricerca del cammino
aumentante andrebbe avanti all'infinito; per questa ragione viene trattato in questa dispensa in termini di numeri naturali, dove nel caso peggiore
si otterrà il flusso massimo in un tempo equivalente al valore del suo modulo, dunque il corpo dell'algoritmo ha complessità $O(V+E)$ ed è eseguito
$|f^*|$\footnote{Notazione usata per indicare il flusso massimo.} volte, per un tempo di esecuzione totale di $O(|f^*|(V+E))$.\par
È chiaro che se il numero di iterazioni da eseguire è molto alto, Ford-Fulkerson ne risulterebbe estremamente inefficiente; per questa ragione è utile
effettuare una visita BFS della rete per trovare il cammino aumentante, ed eseguire l'algoritmo in toto in tempo polinomiale. L'idea è descritta
dall'algoritmo di \textbf{Edmonds-Karp}, il quale, eseguito su una rete di flusso $G$ con sorgente $s$ e pozzo $t$, afferma che per ogni nodo
$v\in V - \{s,t\}$, la distanza del cammino minimo $\delta_f(s,v)$ nella rete residua $G_f$ aumenta monotonicamente per ogni aumento di flusso.\newline

\noindent Per dimostrarlo, supponiamo che un aumento di flusso provochi la diminuzione della distanza del cammino minimo dalla sorgente a qualche nodo
$v\in V - \{s,t\}$ e che da questo derivi una contraddizione.\par
Sia quindi $f$ il flusso prima dell'aumento che riduce la distanza del cammino minimo ed $f'$ quello postumo. Sia poi $v$ un nodo con il minimo
$\delta_f(s,v)$ la cui distanza è stata ridotta dall'aumento facendo valere $\delta_{f'}(s,v) < \delta_f(s,v)$. Definiamo inoltre $p$ un cammino minimo
da $s$ a $v$ tale per cui valgano: \[
	(u,v)\in E\quad \text{ e }\quad \delta_{f'}(s,u) = \delta_{f'}(s,v)-1
\]
\noindent Per la scelta di $v$ sappiamo che la distanza di $u$ dalla sorgente non è diminuita, quindi $\delta_{f'}(s,u) \geq \delta_f(s,u)$, e
asseriamo che l'arco $(u,v)\notin E$, perché se così non fosse, contraddirremmo l'ipotesi iniziale.\par
Quindi com'è possibile che questo arco sia nella rete residua post-aumento e non nella precedente? Sappiamo che l'aumento deve avere incrementato il
flusso da $v$ a $u$, di modo che $(u,v)$ stava nel cammino aumentante. Quest'ultimo è anche un cammino minimo dalla sorgente al pozzo nella rete residua
e siccome un cammino minimo contiene anche i suoi sottocammini minimi, l'aumentante contiene un cammino minimo dalla sorgente a $u$ in $G_f$, la quale
ha $(v,u)$ come suo ultimo arco, facendo valere: \begin{equation}
	\begin{split}
	...
	\end{split}
\end{equation}
\noindent Ma ciò contraddice l'ipotesi iniziale, dunque tale vertice $v$ non esiste e il teorema è dimostrato. Adesso bisogna confermare l'esecuzione
in tempo polinomiale. Diciamo che un arco è critico in un cammino aumentante $p$ se la capacità residua di quest'ultimo è uguale a quella dell'arco,
dunque $c_f(p) = c_f(u,v)$. Dopo l'aumento del flusso lungo il cammino aumentante, ogni arco critico lungo $p$ scompare dalla rete residua e in ogni
cammino aumentante ci deve essere almeno un arco critico.\par
Con la dimostrazione precedente, abbiamo appurato che i cammini aumentanti sono anche minimi; dunque quando l'arco $(u,v)$ diventa critico per la prima
volta, vale: \[
	\delta_f(s,v) = \delta_f(s,u)+1
\]
\noindent Una volta che il flusso è aumentato, l'arco $(u,v)$ scompare dalla rete residua $G_f$ e non vi potrà riapparire finché il flusso da $u$ a $v$
non diminuisce, cosa che accade solo se $(v,u)$ appare in un cammino aumentante. Se $f'$ è il flusso in $G$ quando si verifica quest'ultimo evento,
abbiamo che: \begin{equation}
	...
	\end{split}
\end{equation}
\noindent Quindi dall'istante in cui $(u,v)$ diventa critico all'istante in cui lo ridiventa, la distanza di $u$ dalla sorgente aumenta di almento due
unità. Un arco può diventare critico per un totale di al più $|V|/2$ volte. Poiché ci sono inoltre $O(E)$ coppie di vertici che possono essere connessi
da un arco in una rete residua, il numero totale di archi critici durante l'intera esecuzione di Edmonds-Karp è $O(VE)$; infine, dato che ogni
iterazione di Ford-Fulkerson richiede tempo $O(E)$ quando usa la visita BFS, il tempo totale di esecuzione dell'algoritmo è $O(VE^2)$. Alternativamente,
in base alle caratteristiche del grafo, è possibile scegliere un limite più stretto corrispondente alla complessità di Ford-Fulkerson.

\begin{comment}
--- Problema dell'agenzia matrimoniale.
Siamo dei gestori di un'agenzia matrimoniale dove vengono uomini e donne per trovare l'ammòre. Si intervistano e in base ai dati ottenuti si cerca di
matchare con persone adeguate. Veniamo pagati solo per matching e duraturi.
Notare solo duraturi, non frega della qualità, l'importante è guadagnarci.

Facciamo interviste, abbiamo un'idea delle coppie funzionanti o meno; formiamo il numero più alto di coppie possibili. Come possiamo rappresentare
questo problema? Come possiamo risolverlo?

In maniera formale, abbiamo uomini, donne e coppie che si possono formare. Questa è una relazione. Sì, quella del prodotto cartesiano.
Useremo grafi per rappresentare gli oggetti e archi per la relazione.
Tutti gli omiin da una parte, le donne dall'altra, e inizio a costruirmi le mie relazioni di compatibilità. Questo forma anche un grafo bipartito.
Il problema in questione è detto matching massimale su grafo bipartito.

Come lo risolviamo? Si può ricondurre a un problema di flusso massimo utilizzando un supernodo per gli uomini e un secondo supernodo target dove vanno
tutte le donne.
Il vincolo è che ogni uomo sia vincolato a una sola donna. La capacità dal supernodo e al supernodo fine sarà 1.

Mando quindi un'unità di flusso da un uomo; questo sarà indirizzato verso una sola relazione con una donna, la quale farà uscire il flusso al supernodo
finale.
Se ripetiamo questa operazione con ogni uomo, il valore dei flussi ricevuti dal supernodo finale equivarrà a quello del numero di coppie formate.

In quanto flusso massimo, possiamo usarci ford-fulkerson. Cerchiamo quindi i cammini aumentanti fin quando non se ne trovano più e formiamo le coppie
in base ad esso.
Il problema della ricerca del tagio di capacità minima è il suo duale.
\end{comment}